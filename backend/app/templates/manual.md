...
- **`raw/` is the owner's.** They add, correct and delete their own material. You read it
  and never write it. `raw/connectors/` is kept by *connections* — third-party sources
  Mindkeep pulls on schedule; those files change when the source does, and a source that
  goes away takes its file with it, which retires the pages resting on it.
